...
<div class="container">
    ...
    <h5 class="text-center">Nombre de los 10 municipios de los que menos personas viajan, agregue el nombre del estado.</h5>
    ...
    e.nombre AS estado,
    COUNT(*) AS total_personas_viajan
FROM
    clientes c
INNER JOIN
    municipios m ON c.cve_municipios = m.cve_municipios
INNER JOIN
    estados e ON c.cve_estados = e.cve_estado
GROUP BY
    m.nombre, e.nombre
ORDER BY
    total_personas_viajan ASC;";
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function generarGrafico(tipo) {
                ...
                var etiquetas = data.map(function (elemento) {
                    return elemento.municipio + ' (' + elemento.estado + ')';
                });
                var valores = data.map(function (elemento) {
                    return elemento.total_personas_viajan;
                });
