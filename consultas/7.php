...
<div class="container">
    ...
    <h5 class="text-center">Numero de vuelos por cada aeropuerto de salida en cada año, del aeropuerto se desea saber la clave internacional del aeropuerto y la clave internacional del pais al que pertenece el aeropuerto</h5>
    ...
    a.cve_aeropuertos AS clave_aeropuerto,
    a.clave_internacional AS clave_aeropuerto_internacional,
    p.clave_internacional AS clave_pais_internacional,
...
INNER JOIN
    aeropuertos a ON v.cve_aeropuertos__origen = a.cve_aeropuertos
INNER JOIN
    ciudades c ON a.cve_ciudades = c.cve_ciudades
INNER JOIN
    paises p ON c.cve_paises = p.cve_paises
GROUP BY
    YEAR(dv.fecha_hora_salida),
    a.cve_aeropuertos,
    a.clave_internacional,
    p.clave_internacional
ORDER BY
    año,
    clave_aeropuerto;";
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function generarGrafico(tipo) {
                ...
                var etiquetas = data.map(function (elemento) {
                    return elemento.clave_aeropuerto_internacional + ' (' + elemento.clave_pais_internacional + ')';
                });
                var valores = data.map(function (elemento) {
                    return elemento.numero_vuelos;
                });
